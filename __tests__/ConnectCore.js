...
const CREDENTIALS_JWT =
  "eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NksifQ.eyJyZXF1ZXN0ZWQiOlsibmFtZSIsInBob25lIl0sImlzcyI6IjB4MDAxMTIyIiwiaWF0IjoxNDg1MzIxMTMzOTk2fQ.zxGLQKo2WjgefrxEQWfwm_oago8Qr4YctBJoqNAm2XKE-48bADjolSo2T_tED9LnSikxqFIM9gNGpNgcY8JPdg";
const REQUEST_TOKEN =
  "eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NksifQ.fyJyZXF1ZXN0ZWQiOlsibmFtZSIsInBob25lIl0sImlzcyI6IjB4MDAxMTIyIiwiaWF0IjoxNDg1MzIxMTMzOTk2fQ.zxGLQKo2WjgefrxEQWfwm_oago8Qr4YctBJoqNAm2XKE-48bADjolSo2T_tED9LnSikxqFIM9gNGpNgcY8JPdg";
...
const FAKETX =
  "0x21893aaa10bb28b5893bcec44b33930c659edcd2f3f08ad9f3e69d8997bef238";
...
function mockSigningCredentials({ createRequest, receive }) {
  return {
    ...
    createRequest: payload =>
      new Promise((resolve, reject) => resolve(createRequest(payload))),
    receive: jwt => new Promise(resolve => resolve(receive(jwt)))
  };
}
...
function mockAttestingCredentials(mockfn) {
  return {
    settings: {},
    attest: payload =>
      new Promise((resolve, reject) => resolve(mockfn(payload)))
  };
}
...
describe("ConnectCore", () => {
  describe("config", () => {
    ...
    it("configures credentials correctly", () => {
      ...
      expect(
        uport.credentials.settings.address,
        "uport.credentials.settings.address"
      ).to.equal(CLIENT_ID);
    ...
    it("configures the network in connect and in credentials give a supported string", () => {
      ...
      expect(
        "0x1" in uport.credentials.settings.networks,
        "uport.credentials.settings.networks includes 0x1"
      ).to.be.true;
    });
    ...
    it("configures the network in connect and in credentials given a well formed network config object", () => {
      const netConfig = {
        id: "0x5",
        registry: "0xab6c9051b9a1eg1abc1250f8b0640848c8ebfcg6",
        rpcUrl: "https://somenet.io"
      };
      const uport = new ConnectCore("test app", { network: netConfig });
      expect(uport.network.id, "uport.network.id").to.equal("0x5");
      expect(
        "0x5" in uport.credentials.settings.networks,
        "uport.credentials.settings.networks includes 0x5"
      ).to.be.true;
    });

    it("throws error if the network config object is not well formed ", () => {
      try {
        new ConnectCore("test app", { network: { id: "0x5" } });
      } catch (e) {
        return;
      }
  ...
  describe("request", () => {
    ...
    it("defaults to the preset uriHandler", () => {
      const uriHandler = sinon.spy(),
        closeUriHandler = sinon.spy();
      const uport = new ConnectCore("UportTests", {
        uriHandler,
        closeUriHandler
      });
      return uport.request({ topic: mockTopic(), uri }).then(
        response => {
          expect(response, "uport.request response").to.equal(UPORT_ID);
          expect(uriHandler.calledWith(uri), uriHandler.lastCall.args[0]).to.be
            .true;
          expect(closeUriHandler.called, "closeUriHandler called").to.be.true;
        },
        error => {
          throw new Error(
            "uport.request Promise rejected, expected it to resolve"
          );
        }
      );
    });
    ...
    it("can be overriden by a passed in uriHandler", () => {
      const uriHandler = sinon.spy(),
        closeUriHandler = sinon.spy(),
        uriHandlerDefault = sinon.spy();
      const uport = new ConnectCore("UportTests", {
        uriHandler: uriHandlerDefault,
        closeUriHandler
      });
      return uport
        .request({
        ...
        .then(
          response => {
            ...
            expect(uriHandler.calledWith(uri), uriHandler.lastCall.args[0]).to
              .be.true;
            expect(closeUriHandler.called, "closeUriHandler called").to.be
              .false;
            expect(uriHandlerDefault.called, "default uriHandler called").to.be
              .false;
          },
          error => {
            throw new Error(
    ...
    it("uses the preset mobileUriHandler", () => {
      ...
      return uport.request({ uri, topic: mockTopic() }).then(
        response => {
          expect(response, "uport.request response").to.equal(UPORT_ID);
          expect(
            mobileUriHandler.calledWith(uri),
            mobileUriHandler.lastCall.args[0]
          ).to.be.true;
        },
        error => {
          throw new Error(
    ...
    it("remembers to close if there is an error on the topic", () => {
      ...
      return uport.request({ topic: errorTopic(), uri }).then(
        response => {
          throw new Error(
            "uport.request Promise resolved, expected it to reject"
          );
        },
        error => {
    ...
    it("sends a push notification if push token is available", () => {
      ...
      return uport.request({ topic: mockTopic(), uri }).then(
        response => {
          expect(pushFunc.calledOnce, "uport.credentials.push called").to.be
            .true;
        },
        error => {
          throw new Error(
    ...
    it("don't send a push notification if on mobile", () => {
      ...
      return uport.request({ topic: mockTopic(), uri, uriHandler }).then(
        response => {
          expect(pushFunc.calledOnce, "uport.credentials.push called").to.be
            .false;
          expect(uriHandler.called, "uriHandler called").to.be.true;
  ...
  describe("requestCredentials", () => {
    describe("without signer", () => {
      it("requests public profile", () => {
        ...
        expect(uport.canSign).to.be.false;
        return uport.requestCredentials().then(
          profile => {
            expect(
              uriHandler.calledWith(
                `https://id.uport.me/me?network_id=0x4&label=UportTests&callback_url=https%3A%2F%2Fchasqui.uport.me%2Fapi%2Fv1%2Ftopic%2F123&client_id=${CLIENT_ID}`
              ),
              uriHandler.lastCall.args[0]
            ).to.be.true;
            expect(profile, "uport.requestCredentials profile").to.equal(
              PROFILE
            );
          },
          error => {
      ...
      it("throws error when requesting specific credentials", () => {
        ...
        return uport.requestCredentials({ requested: ["phone"] }).then(
          profile => {
            throw new Error(
              "uport.request Promise resolved, expected it to reject"
            );
          ...
          error => {
            expect(error.message).to.equal(
              "Specific data can not be requested without a signer configured"
            );
          }
        );
      });
      ...
      it("throws error when requesting notifications", () => {
        const uport = new ConnectCore("UportTests");
        expect(uport.canSign, "uport.canSign").to.be.false;
        return uport.requestCredentials({ notifications: true }).then(
          profile => {
            throw new Error(
              "uport.request Promise resolved, expected it to reject"
            );
          ...
          error => {
            expect(error.message).to.equal(
              "Notifications rights can not currently be requested without a signer configured"
            );
          }
        );
      });
      ...
      it("requests public profile", () => {
        ...
        return uport.requestCredentials().then(
          profile => {
            ...
            expect(
              uriHandler.calledWith(
                `https://id.uport.me/me?requestToken=${REQUEST_TOKEN}`
              ),
              uriHandler.lastCall.args[0]
            ).to.be.true;
          },
    ...
    it("requests specific credentials", () => {
      ...
      expect(uport.canSign).to.be.true;
      return uport
        .requestCredentials({ requested: ["phone"], notifications: true })
        .then(
          profile => {
            expect(profile, "uport.requestCredentials profile").to.equal(
              PROFILE
    ...
    it("passes the accountType param in request if initially configured with it", () => {
      ...
      const uport = new ConnectCore("UportTests", {
        ...
        credentials: mockSigningCredentials({
          receive: () => PROFILE,
          createRequest: payload => {
            expect(payload.accountType).to.equal(accountType);
          }
        })
      });
      return uport.requestCredentials().then(
  ...
  describe("requestAddress", () => {
    it("returns address", () => {
      ...
      return uport.requestAddress().then(
        address => {
    ...
    it("returns address for specific network", () => {
      ...
      return uport.requestAddress().then(
        address => {
          expect(address, "uport.requestAddress address").to.equal(
            KOVAN_ADDRESS
          );
          expect(
            uriHandler.calledWith(
              `https://id.uport.me/me?network_id=0x2a&label=UportTests&callback_url=https%3A%2F%2Fchasqui.uport.me%2Fapi%2Fv1%2Ftopic%2F123&client_id=${CLIENT_ID}`
            ),
            uriHandler.lastCall.args[0]
          ).to.be.true;
        },
  ...
  describe("getProvider", () => {
    it("returns a provider with same network settings as connect", () => {
      ...
      expect(uport.network.rpcUrl, "uport.network.rpcUrl").to.equal(
  ...
  describe("attestCredentials", () => {
    ...
    const PAYLOAD = {
      sub: "0x3b2631d8e15b145fd2bf99fc5f98346aecdc394c",
      claim: { name: "Bob" },
      exp: 123123123
    };
    it("provides attestation to user using default uriHandler", () => {
      ...
      return uport.attestCredentials(PAYLOAD).then(
        result => {
          expect(result, "uport.attestCredentials response").to.equal("ok");
          expect(
            uriHandler.calledWith(
              `https://id.uport.me/add?attestations=${ATTESTATION}&callback_url=https%3A%2F%2Fchasqui.uport.me%2Fapi%2Fv1%2Ftopic%2F123`
            ),
            uriHandler.lastCall.args[0]
          ).to.be.true;
        },
  ...
  describe("sendTransaction", () => {
    it("shows simple value url", () => {
      ...
      return uport.sendTransaction({ to: CONTRACT, value: "0xff" }).then(
        txhash => {
          expect(txhash, "uport.sendTransaction txhash").to.equal(FAKETX);
          expect(
            uriHandler.calledWith(
              `https://id.uport.me/2opgxRd4H4WHiZwxGzGQr4HG942X12d39LJ?value=255&label=UportTests&callback_url=https%3A%2F%2Fchasqui.uport.me%2Fapi%2Fv1%2Ftopic%2F123&client_id=${CLIENT_ID}`
            ),
            uriHandler.lastCall.args[0]
          ).to.be.true;
        },
      ...
      return uport
        ...
        .then(
          txhash => {
            ...
            expect(
              uriHandler.calledWith(
                `https://id.uport.me/2opgxRd4H4WHiZwxGzGQr4HG942X12d39LJ?value=255&function=transfer(address%200x3b2631d8e15b145fd2bf99fc5f98346aecdc394c%2Cuint%2012312)&label=UportTests&callback_url=https%3A%2F%2Fchasqui.uport.me%2Fapi%2Fv1%2Ftopic%2F123&client_id=${CLIENT_ID}`
              ),
              uriHandler.lastCall.args[0]
            ).to.be.true;
          },
      ...
      return uport
        ...
        .then(
          txhash => {
            ...
            expect(
              uriHandler.calledWith(
                `https://id.uport.me/2opgxRd4H4WHiZwxGzGQr4HG942X12d39LJ?value=255&bytecode=abcdef01&label=UportTests&callback_url=https%3A%2F%2Fchasqui.uport.me%2Fapi%2Fv1%2Ftopic%2F123&client_id=${CLIENT_ID}`
              ),
              uriHandler.lastCall.args[0]
            ).to.be.true;
          },
      ...
      return uport
        ...
        .then(
          txhash => {
            ...
            expect(
              uriHandler.calledWith(
                `https://id.uport.me/2opgxRd4H4WHiZwxGzGQr4HG942X12d39LJ?value=255&bytecode=abcdef01&label=UportTests&callback_url=https%3A%2F%2Fchasqui.uport.me%2Fapi%2Fv1%2Ftopic%2F123&client_id=${CLIENT_ID}&gasPrice=0x4444`
              ),
              uriHandler.lastCall.args[0]
            ).to.be.true;
          },
  ...
  describe("contract", () => {
    ...
    it("shows correct uri to default uriHandler", () => {
      ...
      return token
        .transfer("0x3b2631d8e15b145fd2bf99fc5f98346aecdc394c", 12312)
        .then(
          txhash => {
            expect(txhash, "token.transfer txhash").to.equal(FAKETX);
            expect(
              uriHandler.calledWith(
                `https://id.uport.me/2opgxRd4H4WHiZwxGzGQr4HG942X12d39LJ?function=transfer(address%200x3b2631d8e15b145fd2bf99fc5f98346aecdc394c%2C%20uint256%2012312)&label=UportTests&callback_url=https%3A%2F%2Fchasqui.uport.me%2Fapi%2Fv1%2Ftopic%2F123&client_id=${CLIENT_ID}`
              ),
              uriHandler.lastCall.args[0]
            ).to.be.true;
          },
    ...
    it("shows correct uri to overridden uriHandler", () => {
      const overideUriHandler = sinon.spy(),
        closeUriHandler = sinon.spy();
      const uport = new ConnectCore("UportTests", {
        clientId: CLIENT_ID,
        topicFactory: name => {
      ...
      const token = uport
        .contract(miniTokenABI)
        .at("0x819320ce2f72768054ac01248734c7d4f9929f6c");
      return token
        .transfer(
          "0x3b2631d8e15b145fd2bf99fc5f98346aecdc394c",
          12312,
          overideUriHandler
        )
        .then(
          txhash => {
            expect(txhash, "token.transfer txhash").to.equal(FAKETX);
            expect(
              overideUriHandler.calledWith(
                `https://id.uport.me/2opgxRd4H4WHiZwxGzGQr4HG942X12d39LJ?function=transfer(address%200x3b2631d8e15b145fd2bf99fc5f98346aecdc394c%2C%20uint256%2012312)&label=UportTests&callback_url=https%3A%2F%2Fchasqui.uport.me%2Fapi%2Fv1%2Ftopic%2F123&client_id=${CLIENT_ID}`
              ),
              overideUriHandler.lastCall.args[0]
            ).to.be.true;
          },
          error => {
            throw new Error(
    ...
    it("MNID encodes contract addresses in requests", () => {
      const uport = new ConnectCore("UportTests");
      const sendTransaction = sinon
        .stub(uport, "request")
        .callsFake(({ uri }) => {
          expect(uri, "request consumes uri").to.match(
            /2opgxRd4H4WHiZwxGzGQr4HG942X12d39LJ/
          );
        });
      const token = uport
        .contract(miniTokenABI)
    ...
    it("accepts contracts at both addresses and MNID encoded adresses", () => {
      ...
      const sendTransactionMNID = sinon
        .stub(uportMNID, "request")
        .callsFake(stubFunc);
